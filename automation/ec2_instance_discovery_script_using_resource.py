...
for each in ec2_con_re.instances.all():
    print(cnt, each.instance_id, each.instance_type, each.architecture, each.launch_time.strftime('%Y-%m-%d'),
          each.private_ip_address)
    csv_w.writerow([cnt, each.instance_id, each.instance_type, each.architecture, each.launch_time.strftime('%Y-%m-%d'),
                    each.private_ip_address])
    cnt += 1
csv_ob.close()
